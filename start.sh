...
# use vp9_vaapi instead of h264_vaapi in order to support old version of nextcloud-spreed-signaling that don't know about mp4 extension, only webm and ogg
elif [ -r "/dev/dri/renderD128" ] && ffmpeg -hide_banner -encoders 2>/dev/null | grep -q "vp9_vaapi"; then
    echo "DRI device detected, enabling vp9_vaapi hardware transcoding"
    FFMPEG_SECTION="[ffmpeg]
common = ffmpeg -loglevel level+warning -n -vaapi_device /dev/dri/renderD128
outputvideo = -vf format=nv12,hwupload -c:v vp9_vaapi -b:v 2M
outputaudio = -c:a libopus
extensionaudio = .ogg
extensionvideo = .webm"
fi
